    ...
    <div className="flex min-h-screen items-center justify-center bg-white px-4">
      <div className="w-full max-w-sm animate-fade-in-up">
        <div className="mb-8 text-center">
          ...
          <p className="mt-1 text-sm text-[#6B6B6B]">Create your account. It&apos;s free.</p>
        </div>

        <div className="rounded-md bg-white p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="name" className="block text-sm font-medium text-[#2D2D2D]">
                Name
              </label>
              <input
                id="name"
        ...
        <p className="mt-4 text-center text-sm text-[#6B6B6B]">
          Already have one?{" "}
          <Link href={callbackUrl !== "/workspaces" ? `/login?callbackUrl=${encodeURIComponent(callbackUrl)}` : "/login"} className="font-medium text-[#4F46E5] hover:text-[#4338CA]">
            Sign in
          </Link>
        </p>
      </div>
